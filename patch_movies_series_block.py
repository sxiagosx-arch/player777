...
for screen in ['MoviesScreen.kt', 'SeriesScreen.kt']:
    path = f"app/src/main/java/com/example/ui/screens/{screen}"
    with open(path, 'r') as f:
        content = f.read()

    # Find the target for injection: "var selectedCategoryId by remember { mutableStateOf("") }"
    target = '    var selectedCategoryId by remember { mutableStateOf("") }'
    replace = """    var selectedCategoryId by remember { mutableStateOf("") }
    ...
    if "showPinDialogForCat" not in content:
